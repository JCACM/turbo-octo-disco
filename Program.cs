...
namespace ConsoleApplication
{
    public class Program
    {
        public struct StockResult {
            ...
            public bool month;
            public bool dayPoss;
            public bool weekPoss;
            public bool monthPoss;

            public StockResult(string symbol, bool day, bool week, bool month, bool dayPoss, bool weekPoss, bool monthPoss)
            {
                this.symbol = symbol;
                this.day = day;
                this.week = week;
                this.month = month;
                this.dayPoss = dayPoss;
                this.weekPoss = weekPoss;
                this.monthPoss = monthPoss;
            }
        }
        ...
        {
            IEnumerable<string> symbols = new string[] {
            ...
            while (symbols.Any()) {
                var tasks = symbols.Take(chunkSize).Select(symbol => ProcessStockSymbol(symbol)).ToList();
                symbols = symbols.Skip(chunkSize);
                results.AddRange(await Task.WhenAll(tasks));
            }

            // var results = await Task.WhenAll(tasks);
            var dayWinners = results.Where(stockResult => stockResult.day == true).Select(stockResult => stockResult.symbol).ToList();
            var weekWinners = results.Where(stockResult => stockResult.week == true).Select(stockResult => stockResult.symbol).ToList();
            var monthWinners = results.Where(stockResult => stockResult.month == true).Select(stockResult => stockResult.symbol).ToList();

            var dayPoss = results.Where(stockResult => stockResult.dayPoss == true).Select(stockResult => stockResult.symbol).ToList();
            var weekPoss = results.Where(stockResult => stockResult.weekPoss == true).Select(stockResult => stockResult.symbol).ToList();
            var monthPoss = results.Where(stockResult => stockResult.monthPoss == true).Select(stockResult => stockResult.symbol).ToList();
        ...
        static async Task<StockResult> ProcessStockSymbol(string symbol)
        {
